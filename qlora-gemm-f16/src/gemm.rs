...
#[cfg(feature = "std")]
use qlora_gemm_common::gemm::L2_SLAB;
#[cfg(feature = "rayon")]
use qlora_gemm_common::gemm::{get_threading_threshold, par_for_each};

use qlora_gemm_common::{
...
pub mod f16 {
    ...
    use qlora_gemm_common::Parallelism;
    ...
    fn init_gemm_fn() -> GemmTy {
        ...
        {
            #[cfg(feature = "x86-v4")]
            if qlora_gemm_common::feature_detected!("avx512f") {
                return avx512f::gemm_basic;
            }
            if qlora_gemm_common::feature_detected!("fma") {
                fma::gemm_basic
            } else {
                scalar::gemm_basic
            }
        ...
        {
            if qlora_gemm_common::feature_detected!("neon") {
                #[cfg(feature = "experimental-apple-amx")]
                if qlora_gemm_common::cache::HasAmx::get() {
                    return amx::gemm_basic;
                }
                if qlora_gemm_common::feature_detected!("fp16") {
                    neonfp16::gemm_basic
                } else {
                    neon::gemm_basic
                }
    ...
    mod scalar {
        use super::*;
        use qlora_gemm_common::simd::Scalar;
        use qlora_gemm_f32::microkernel::scalar::f32::*;
        const N: usize = 1;
        ...
        pub unsafe fn gemm_basic(
            ...
            _conj_lhs: bool,
            _conj_rhs: bool,
            parallelism: qlora_gemm_common::Parallelism,
        ) {
    ...
    mod neon {
        use super::*;
        use qlora_gemm_common::simd::MixedSimd;
        use qlora_gemm_f32::microkernel::neon::f32::*;
        const N: usize = 4;
        ...
        pub unsafe fn gemm_basic(
            ...
            gemm_basic_generic::<N, { MR_DIV_N * N }, NR, MR_DIV_N, _>(
                qlora_gemm_common::simd::Neon::try_new().unwrap(),
                m,
                n,
                k,
                dst,
    ...
    mod neonfp16 {
        ...
        use qlora_gemm_common::simd::{MixedSimd, NeonFp16};
        type T = half::f16;
        ...
        pub unsafe fn gemm_basic(
            ...
            qlora_gemm_common::gemm::gemm_basic_generic::<_, _, N, { MR_DIV_N * N }, NR, MR_DIV_N, 0, 0>(
                simd,
                m,
                n,
                k,
    ...
    mod fma {
        use super::*;
        use qlora_gemm_common::simd::V3;
        use qlora_gemm_f32::microkernel::fma::f32::*;
        const N: usize = 8;
    ...
    mod avx512f {
        use super::*;
        use qlora_gemm_common::simd::V4;
        use qlora_gemm_f32::microkernel::avx512f::f32::*;
        const N: usize = 16;
